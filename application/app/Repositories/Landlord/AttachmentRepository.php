...
class AttachmentRepository {
    ...
    public function processAppLogo($data = []) {
        ...
        //sanity
        if ($data["directory"] == '' || $data["filename"] == '' || !in_array($data["logo_size"], ['logo-small', 'logo-large', 'frontend-logo', 'favicon', 'frontend-favicon'])) {
        ...
        $new_file = BASE_DIR . "/storage/logos/app/" . $data["new_filename"];

        //landord - large logo
        if ($data["logo_size"] == 'logo-large') {
            $current_file = BASE_DIR . "/storage/logos/app/" . config('system.settings_system_logo_large_name');
        }

        //landord - small logo
        if ($data["logo_size"] == 'logo-small') {
            $current_file = BASE_DIR . "/storage/logos/app/" . config('system.settings_system_logo_small_name');
        }

        //landord - favicon
        if ($data["logo_size"] == 'favicon') {
            $current_file = BASE_DIR . "/storage/logos/app/" . config('system.settings_favicon_landlord_filename');
        }

        //frontend - logo
        if ($data["logo_size"] == 'frontend-logo') {
            $current_file = BASE_DIR . "/storage/logos/app/" . config('system.settings_system_logo_frontend_name');
        }

        //frontend - favicon
        if ($data["logo_size"] == 'frontend-favicon') {
            $current_file = BASE_DIR . "/storage/logos/app/" . config('system.settings_favicon_frontend_filename');
        }

        //validation: file exists
        if (!file_exists($uploaded_file)) {
            Log::error("the logo file could not be found", ['process' => '[attachments-app-logo]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'path' => $uploaded_file] ?? '');
            return false;
        }

        //check if the file is an image
        if (!is_array(@getimagesize($uploaded_file))) {
            Log::error("the logo file is not an image", ['process' => '[attachments-app-logo]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'path' => $uploaded_file] ?? '');
            return false;
        }

        //delete the old logo (but not system logo)
        @unlink($current_file);
