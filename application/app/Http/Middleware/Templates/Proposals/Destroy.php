...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $proposal->proposal_id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('proposal'))) {
            $ids[$request->route('proposal')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }
        ...
        if (is_array(request('ids'))) {
            ...
            Log::error("no items were sent with this request", ['process' => '[proposals][action-bar]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'proposal id' => $proposal_id ?? '']);
            abort(409);
        }
