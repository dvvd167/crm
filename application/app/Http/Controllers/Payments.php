...
class Payments extends Controller {
    ...
    public function store(PaymentStoreUpdate $request, InvoiceRepository $invoicerepo) {
        ...
        //[AFFILIATE] - record payment
        if (config('settings.custom_modules.cs_affiliate')) {
            $invoicerepo->processAffiliateEarnings($invoice->bill_invoiceid);
        }
    ...
    /**
     * display a thank you for your payment
     * record this payment as if its a webhook and process/validate it via cron
     * @return \Illuminate\Http\Response
     */
    public function thankYouTap(TapPaymentRepository $taprepo) {
        ...
        //missing session id
        if (!request()->filled('session_id') || !request()->filled('tap_id')) {
            return redirect('/home');
        }

        //avoid duplicate
        if (\App\Models\Webhook::Where('webhooks_gateway_name', 'tap')
            ->Where('webhooks_payment_transactionid', request('tap_id'))
            ->Where('webhooks_matching_attribute', request('session_id'))->exists()) {
            return redirect('/home');
        }
