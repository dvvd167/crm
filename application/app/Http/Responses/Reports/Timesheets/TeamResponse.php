...
namespace App\Http\Responses\Reports\Timesheets;
    ...
    public function toResponse($request) {
        ...
        if (request('action') == 'load' || request('action') == 'sort') {
            ...
            if (request('action') == 'load') {
                $html = view('pages/reports/timesheets/team/table', compact('timesheets', 'totals'))->render();
                $jsondata['dom_html'][] = [
            ...
            if (request('action') == 'sort') {
                $html = view('pages/reports/timesheets/team/ajax', compact('timesheets', 'totals'))->render();
                $jsondata['dom_html'][] = [
            ...
            $html = view('pages/reports/timesheets/team/wrapper', compact('timesheets', 'totals'))->render();
            $jsondata['dom_html'][] = [
        ...
        //actions panel
        $html = view('pages/reports/timesheets/components/actions', compact('page'))->render();
        $jsondata['dom_html'][] = [
        ...
        $jsondata['dom_classes'][] = [
            'selector' => '#reports_tabs_timesheets',
            'action' => 'add',
            'value' => 'active',
        ];
