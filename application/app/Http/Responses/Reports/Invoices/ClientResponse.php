...
namespace App\Http\Responses\Reports\Invoices;
use Illuminate\Contracts\Support\Responsable;

class ClientResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('action') == 'load' || request('action') == 'sort') {
            ...
            if (request('action') == 'load') {
                $html = view('pages/reports/invoices/client/table', compact('invoices', 'totals'))->render();
                $jsondata['dom_html'][] = [
            ...
            if (request('action') == 'sort') {
                $html = view('pages/reports/invoices/client/ajax', compact('invoices', 'totals'))->render();
                $jsondata['dom_html'][] = [
            ...
            $html = view('pages/reports/invoices/client/wrapper', compact('invoices', 'totals'))->render();
            $jsondata['dom_html'][] = [
        ...
        //actions panel
        $html = view('pages/reports/invoices/components/actions', compact('page'))->render();
        $jsondata['dom_html'][] = [
        ...
        $jsondata['dom_classes'][] = [
            'selector' => '#reports_tabs_invoices',
            'action' => 'add',
            'value' => 'active',
        ];
