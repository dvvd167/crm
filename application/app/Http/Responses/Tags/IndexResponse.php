...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/tags/components/table/ajax';
                $dom_container = '#tags-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/tags/components/table/wrapper';
                $dom_container = '#tags-table-wrapper';
                $dom_action = 'replace';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/tags/tabswrapper';
            ...
            if ($tags->currentPage() < $tags->lastPage()) {
                $url = loadMoreButtonUrl($tags->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'tags', 'resource_types'))->render();
