        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            if (request('action') == 'load') {
                ...
                if (request()->filled('source')) {
                    ...
                    //append a new button
                    $board['load_more_url'] = loadMoreButtonUrl($tasks->currentPage() + 1, request('source'));
                    $html = view('pages/tasks/components/kanban/loadmore-button', compact('board'))->render();
                    $jsondata['dom_html'][] = [
                        'selector' => '#kanban-board-wrapper-' . $board_id,
                        'action' => 'append',
                        'value' => $html,
            ...
            //standard view
            return view('pages/tasks/wrapper', compact('page', 'boards', 'stats', 'tags', 'milestones', 'statuses', 'fields'))->render();
        }
    }
}
