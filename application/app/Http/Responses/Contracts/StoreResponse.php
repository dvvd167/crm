...
class StoreResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('show_after_adding') == 'on') {
            ...
            $jsondata['redirect_url'] = url("/contracts/$id/edit");

        } else {
            //prepend content on top of list or show full table
            if ($count == 1) {
                $html = view('pages/contracts/components/table/table', compact('contracts'))->render();
                $jsondata['dom_html'][] = array(
                    'selector' => '#contracts-table-wrapper',
                    'action' => 'replace',
                    'value' => $html);
            } else {
                ...
                $jsondata['dom_html'][] = array(
                    'selector' => '#contracts-td-container',
                    'action' => 'prepend',
                    'value' => $html);
            }
