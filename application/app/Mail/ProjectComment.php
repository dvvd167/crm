...
/** --------------------------------------------------------------------------------
 * This classes renders the [project comment] email and stores it in the queue
...
class ProjectComment extends Mailable {
    use Queueable;
    ...
    public function build() {
        ...
        //email template
        if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Project Comment')->first()) {
            return false;
        }
        ...
        $payload += [
            ...
            'poster_first_name' => $this->data['first_name'],
            'poster_last_name' => $this->data['last_name'],
            ...
            'project_comments_url' => url('/projects/' . $this->obj->project_id . '/comments'),
        ];
