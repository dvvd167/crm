...
/** --------------------------------------------------------------------------------
 * This classes renders the [decline estimate] email and stores it in the queue
...
class DeclineEstimate extends Mailable {
    use Queueable;
    ...
    public function build() {
        ...
        //email template
        if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Estimate Declined')->first()) {
            return false;
        }
        ...
        $payload += [
            ...
            'estimate_url' => url('/estimates/view/' . $this->obj->bill_uniqueid),
        ];
