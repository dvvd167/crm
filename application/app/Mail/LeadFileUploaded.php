...
/** --------------------------------------------------------------------------------
 * This classes renders the [lead file upload] email and stores it in the queue
...
class LeadFileUploaded extends Mailable {
    use Queueable;
    ...
    public function build() {
        ...
        //email template
        if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Lead File Uploaded')->first()) {
            return false;
        }
        ...
        $payload += [
            ...
            'file_url' => url('leads/download-attachment/' . $this->data['attachment_uniqiueid']),
