...
/** --------------------------------------------------------------------------------
 * This classes renders the [payment received] email and stores it in the queue
...
class PaymentReceived extends Mailable {
    ...
    public function build() {
        ...
        if ($this->user->type == 'team') {
            if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'New Payment')->first()) {
                return false;
            }
        }
        ...
        if ($this->user->type == 'client') {
            if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Thank You For Payment')->first()) {
                return false;
            }
        }
        ...
        $payload += [
            ...
            'paid_by_name' => $this->data['paid_by_name'],
